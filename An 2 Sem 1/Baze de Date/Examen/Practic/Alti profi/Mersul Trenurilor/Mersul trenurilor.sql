...
INSERT INTO Rute (nume, id_tren) VALUES ('Cluj - Asia', 2) -- 7
...
-- Am incurcat sosirea cu plecarea, my bad, nu mai am chef sa le refac
...
-- Verificare
EXECUTE dbo.AddIntersection @id_statie=2, @id_ruta=7, @ora_plecarii='07:59:59', @ora_sosirii='08:00:00'
EXECUTE dbo.AddIntersection @id_statie=2, @id_ruta=7, @ora_plecarii='08:00:00', @ora_sosirii='07:59:59'
...
-- 4. Functie
GO
CREATE OR ALTER FUNCTION uf_MulteTrenuri(@ora INT)
RETURNS TABLE 
AS
RETURN 
	SELECT s.nume, COUNT(DISTINCT r.id_tren) AS NumarTrenuri FROM Statii s
	INNER JOIN Intersectii i ON i.id_statii = s.id_s
	INNER JOIN Rute r ON i.id_rute = r.id_r
	WHERE @ora <= DATEPART(HOUR, i.ora_plecarii) AND @ora >= DATEPART(HOUR,i. ora_sosirii)
	GROUP BY s.id_s, s.nume
	HAVING COUNT(DISTINCT r.id_tren) > 1
GO

SELECT * FROM uf_MulteTrenuri(8)

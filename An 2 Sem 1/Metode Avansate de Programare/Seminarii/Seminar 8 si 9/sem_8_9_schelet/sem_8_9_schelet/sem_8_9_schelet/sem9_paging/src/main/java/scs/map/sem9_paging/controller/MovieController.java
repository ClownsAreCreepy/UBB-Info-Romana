...
import scs.map.sem9_paging.dto.MovieFilterDTO;
...
public class MovieController implements Observer<EntityChangeEvent> {
    ...
    private final MovieFilterDTO movieFilter = new MovieFilterDTO();
    ...
    private void initYearsCombo() {
        var years = movieService.getYears();
        years.addFirst(null);
    ...
    public void initialize() {
        ...
        textFieldfilterTitle.textProperty().addListener(o->{
            movieFilter.setTitle(textFieldfilterTitle.getText());
            currentPage = 0;
            initModel();
        });

        textFieldfilterDirector.textProperty().addListener(o->{
            movieFilter.setDirector(textFieldfilterDirector.getText());
            currentPage = 0;
            initModel();
        });

        textFieldfilterYearAfter.textProperty().addListener(o->{
            String yearAfter = textFieldfilterYearAfter.getText();
            Integer year;

            try{
                year = Integer.parseInt(yearAfter);
            }catch(NumberFormatException e){
                year = null;
            }

            movieFilter.setYearAfter(year);
            currentPage = 0;
            initModel();
        });

        comboBoxfilterYear.getSelectionModel().selectedItemProperty().addListener(o->{
            movieFilter.setYear(comboBoxfilterYear.getSelectionModel().getSelectedItem());
            currentPage = 0;
            initModel();
        });
    ...
    private void initModel() {
        Page<Movie> page = movieService.findAllOnPage(new Pageable(currentPage, pageSize),movieFilter);
        ...
        if(currentPage > maxPage){
            currentPage = maxPage;
            page =  movieService.findAllOnPage(new Pageable(currentPage, pageSize),movieFilter);
        }
        ...
        var movies = StreamSupport.stream(page.getElementsOnPage().spliterator(), false)
    ...
    public void onEdit(ActionEvent actionEvent) throws IOException {
        ...
        controller.init(movieService, stage, selected);

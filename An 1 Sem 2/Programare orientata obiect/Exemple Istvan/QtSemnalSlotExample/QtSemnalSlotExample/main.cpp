...
QWidget* createButtons(QApplication &a) {
	...
	QObject::connect(store, &QPushButton::clicked, [&a]() {
		QMessageBox::information(nullptr, "Info", "Butonul store apasat");
	});
...
QWidget* createSpinner() {
	...
	QObject::connect(slAge, &QSlider::valueChanged, [spAge,slAge](int val) {
		spAge->setValue(val); }
	);
